...
jQuery(document).ready(function () {
    ...
    var key = 'chartivio_autosave_' + cvioSettings.postId;
    var raw = localStorage.getItem(key);
    if (raw) {
        try {
            var local_snapshot = JSON.parse(raw);
            if (!snapshotsEqual(local_snapshot, cvioSettings.savedSnapshot)) {
                // restore from local
...
function chartivio_local_autosave(init) {
    ...
    try {
        var savedRaw = JSON && cvioSettings.savedSnapshot ? JSON.stringify(cvioSettings.savedSnapshot) : '';
