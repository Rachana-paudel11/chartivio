...
function chartivio_admin_assets($hook)
{
    ...
    if ($hook == 'post-new.php' || $hook == 'post.php') {
        if ($post && $post->post_type === 'chartivio') {
            ...
            wp_enqueue_script('chartivio-admin-settings', plugins_url('../assets/js/admin-settings.js', __FILE__), array('jquery', 'chartjs'), '1.0.1', true);
            ...
            $saved_snapshot = array(
                ...
                'csv_url' => $csv_url,
                'csv_data' => get_post_meta($post->ID, '_chartivio_csv_data', true) ?: null,
            ...
            wp_add_inline_script('chartivio-admin-settings', 'const cvioSettings = ' . wp_json_encode($settings_data) . ';', 'before');
        }
    }
}
...
function chartivio_render_main_box($post)
{
    ...
    <div class="cvio-admin-wrapper">
        ...
        <div class="cvio-split-container">
            ...
            /* Inline Script Moved to assets/js/admin-settings.js */
            <?php
}
